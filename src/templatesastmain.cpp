...
int main(int argc, const char *argv[]) {
    ...
    ExpList<float> *l3 = nullptr;
    l3 = new ExpList<float>(new Constant<float>(.222), l3);
    l3 = new ExpList<float>(new Constant<float>(3.0), l3);
    Exp<float> *e3 = new OpApp<float>(floatops, l2);

    Exp<float> *in = new OpApp<float>(floatops,
                                      new ExpList<float>(
                                              new Var<float>("hiya"),
                                              new ExpList<float>(
                                                      new Constant<float>(2.0),
                                                      nullptr
                                              )
                                      )
    );
    Exp<float> *let1 = new Let<float>("hiya", e3, in);
    cout << let1->eval(nullptr) << endl; // 1.332
}